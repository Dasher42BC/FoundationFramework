...
# 	Created:	??/??/?? -	??
#
# 	Modified:	09/26/02 -      by Capt. Redbeard
#
# 	(disabled mainmenu testmode checks so mainmenu always
#       loads as if game were started in testmode)
#
# 	Modified:	09/01/03 -      by Defiant <mail@defiant.homedns.org> (Scrolling Mutators)
#
###############################################################################
...
resheight = App.g_kIconManager.GetScreenHeight()
...
def HandleTestGameTab(pWindow, pEvent):
    SwitchMiddlePane("Mission Select")
...
def SetHandlers():
    ...
    App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_UI_SWITCH_ICON_GROUPS, pOptionsWindow, __name__ + ".HandleSwitchIconGroups")
    App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_UI_REPOSITION, pOptionsWindow, __name__ + ".HandleSwitchRes")
    # 	App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_ST_LOAD_DIALOG_LOAD,
    # 												  	pOptionsWindow, __name__ + ".HandleLoadGameButton")
    App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_GAME_LOADED, pOptionsWindow, __name__ + ".GameLoaded")
...
def SwitchMiddlePane(pcNewPaneName, bGameEnded=0):
    ...
    if pcNewPaneName == "New Game":
    ...
    elif pcNewPaneName == "Mission Select":
        ...
        pPane = App.STLoadDialog_Create(LCARS.MAIN_MENU_MIDDLE_PANE_WIDTH, LCARS.MAIN_MENU_MIDDLE_PANE_HEIGHT)
        ...
        pPane = App.STSaveDialog_Create(LCARS.MAIN_MENU_MIDDLE_PANE_WIDTH, LCARS.MAIN_MENU_MIDDLE_PANE_HEIGHT)
        pMenu = pPane.GetFileMenu()
...
def RebuildMiddleBackground(kString):
    ...
    pMiddleArea.AddChild(pBackground, LCARS.MAIN_MENU_MIDDLE_PANE_X, LCARS.MAIN_MENU_MIDDLE_PANE_Y)
...
def BuildMainMenuTopSection():
    ...
    pTopPane.AddChild(pButtonPane, LCARS.MAIN_MENU_TOP_BUTTON_PANE_X, LCARS.MAIN_MENU_TOP_BUTTON_PANE_Y, 0)
    ...
    pTopBackgroundPane.AddChild(pVersion, fLeft + pThing.GetWidth() - pVersion.GetWidth() - (App.globals.DEFAULT_ST_INDENT_HORIZ * 8.0), pThing.GetTop(), 0)
...
def BuildMainMenuTopButtonPane(pEventHandlerObject):
    ...
    pButtonPane = App.TGPane_Create(LCARS.MAIN_MENU_TOP_BUTTON_PANE_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_PANE_HEIGHT)
    ...
    pNewGameButton = App.STRoundedButton_CreateW(pDatabase.GetString("New Game"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pMultiplayerButton = App.STRoundedButton_CreateW(
        pDatabase.GetString("Multiplayer"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT
    )
    ...
    pConfigureButton = App.STRoundedButton_CreateW(
        pDatabase.GetString("Configure"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT
    )
    ...
    pEvent.SetDestination(pEventHandlerObject)
    pQuitButton = App.STRoundedButton_CreateW(pDatabase.GetString("Quit"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pLoadGameButton = App.STRoundedButton_CreateW(pDatabase.GetString("Load Game"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    if App.g_kUtopiaModule.GetTestMenuState() == 0:
        ...
        pEvent.SetDestination(pOptionsWindow)
        pTestOnlyButton = App.STRoundedButton_CreateW(
            pDatabase.GetString("Quick Battle"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT
        )
    else:
        pEvent = App.TGEvent_Create()
        pEvent.SetSource(pButtonPane)
        ...
        pTestOnlyButton = App.STRoundedButton_Create("Mission Select", pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pCreditsButton = App.STRoundedButton_CreateW(pDatabase.GetString("Credits"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pResumeButton = App.STRoundedButton_CreateW(pDatabase.GetString("Resume"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
...
def HandleKeyboardTopButtonArea(pButtonPane, pEvent):
    ...
    if eKeyType == App.TGKeyboardEvent.KS_KEYUP:
        ...
        kButtonMapping = [pNewGame, pConfigure, pMultiplayer, pQuit, pLoadGame, pCredits, pQuickBattle, pResume]
...
def BuildMiddlePane(kName):
    pPane = App.TGPane_Create(1.0, 1.0 - 0.17 - g_fVerticalSpacing * 2.0)
...
def BuildMiddleBackground(kName, bListPane=0):
    ...
    pText.SetColor(App.g_kSTMenuTextColor)
    pPane.AddChild(
        pText, 1.0 - g_fHorizontalSpacing - pText.GetWidth() - g_fHorizontalSpacing * 2.0, fTop + pThing2.GetHeight() - (pText.GetHeight() * 1.25), 0
    )
    ...
    pText.SetColor(App.g_kSTMenuTextColor)
    pPane.AddChild(
        pText, fLeft + pCurve.GetWidth() - pText.GetWidth() - g_fHorizontalSpacing, fTop + pCurve.GetHeight() - pText.GetHeight() - g_fVerticalSpacing / 2.0, 0
    )
    ...
    pPane.AddChild(
        pThing,
        fLeft + pCurve.GetWidth() + g_fHorizontalSpacing,
        fTop + pOuterIcon.GetHeight() + g_fVerticalSpacing + pCurve.GetHeight() - pThing.GetHeight(),
        0,
    )
...
def BuildNewGamePane():
    ...
    pNewGamePane = App.TGPane_Create(LCARS.MAIN_MENU_MIDDLE_PANE_WIDTH, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT)
    ...
    pTabPane = App.TGPane_Create(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT)
    App.g_kFocusManager.AddObjectToTabOrder(pTabPane)

    pContentPane.AddChild(pTabPane, LCARS.MAIN_MENU_CONFIGURE_TAB_X, LCARS.MAIN_MENU_CONFIGURE_TAB_Y - 0.01)
    ...
    pTabPane.AddChild(pButton, 0.0, fTop - 0.06, 0)
    ...
    pTabPane.AddChild(pTextEntry, fWidth + 0.02, fTop - (LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT - fHeight) / 2.0 - 0.06, 0)

    pPane = App.TGPane_Create(1.0, 1.0)
    pTabPane.AddChild(pPane, 0, -0.06, 0)
    ...
    pButton.SetDisabled(0)
    pButton.SetColorBasedOnFlags()
    pTabPane.AddChild(pButton, 0.365, fTop - 0.08, 0)
    ...
    pTabPane.AddChild(pButton, fWidth + 0.375, fTop - 0.08, 0)
...
def BuildTestGamePane():
    ...
    pNewGamePane = App.TGPane_Create(
        LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 2.0 + LCARS.MAIN_MENU_MIDDLE_CONTENT_X, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT + LCARS.MAIN_MENU_MIDDLE_CONTENT_Y
    )
    pNewGameSubPane = App.STSubPane_Create(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 2.0, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT)
    ...
    pNewGameStylized.SetMaximumSize(
        LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 2.0,
    ...
    try:
        ...
        pNewGameSubPane.AddChild(Local.CreateGrrGameButton())
    except:
        pass
...
def BuildTestGamePane():
    ...
    pNewGamePane = App.TGPane_Create(
        LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 1.08 + LCARS.MAIN_MENU_MIDDLE_CONTENT_X - 0.1,
        LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT + LCARS.MAIN_MENU_MIDDLE_CONTENT_Y,
    )
    pNewGameSubPane = App.STSubPane_Create(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 2.0, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT, 1)
    pNewGameStylized = App.STStylizedWindow_Create("StylizedWindow", "NoMinimize", "Missions", 0.0, 0.0)
    pNewGameStylized.SetTitleBarThickness(0.029)
    pNewGameStylized.SetFixedSize(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 2.1, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT / 1.2)
    pNewGameStylized.Resize(pNewGameStylized.GetMaximumWidth(), pNewGameStylized.GetMaximumHeight())
    pNewGameStylized.AddChild(pNewGameSubPane)
    # pNewGameStylized.SetMaximumSize(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH / 3.0,
    # LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT + pNewGameStylized.GetBorderHeight())
    # 								0.65)
    App.g_kFocusManager.AddObjectToTabOrder(pNewGameSubPane)
    ...
    #############################################################
    # Custom missions
    pCustomMenu = App.STMenu_CreateW(pDatabase.GetString("Custom missions"))

    try:
        import nt

        lFiles = nt.listdir("scripts/Custom/Games")
        # Check which of these entries are directories.
        for sCandidate in lFiles:
            if ((nt.stat("scripts/Custom/Games/" + sCandidate)[0] & 0170000) == 0040000):
                # Directory.
                try:
                    # Check if the campaign wants to create its own menu/button.
                    sModuleName = "Custom.Games." + sCandidate + "." + sCandidate
    ...
    pNewGamePane.AddChild(pNewGameStylized, LCARS.MAIN_MENU_MIDDLE_CONTENT_X + 0.33, LCARS.MAIN_MENU_MIDDLE_CONTENT_Y)
...
def BuildConfigurePane(bGameEnded=0):
    ...
    pTabPane = App.TGPane_Create(LCARS.MAIN_MENU_CONFIGURE_TAB_WIDTH, LCARS.MAIN_MENU_CONFIGURE_TAB_HEIGHT)
    App.g_kFocusManager.AddObjectToTabOrder(pTabPane)
    ...
    pOptionsPane = App.STSubPane_Create(LCARS.MAIN_MENU_CONFIGURE_CONTENT_WIDTH, LCARS.MAIN_MENU_CONFIGURE_CONTENT_HEIGHT)
    ...
    pGeneralTab = App.STRoundedButton_CreateW(
        pDatabase.GetString("General Options"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT
    )
    ...
    pEvent.SetObjPtr(pOptionsPane)
    pSoundTab = App.STRoundedButton_CreateW(pDatabase.GetString("Sound"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pEvent.SetObjPtr(pOptionsPane)
    pGraphicsTab = App.STRoundedButton_CreateW(pDatabase.GetString("Graphics"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT)
    ...
    pEvent.SetObjPtr(pOptionsPane)
    pKeyboardTab = App.STRoundedButton_CreateW(
        pDatabase.GetString("ControlsShort"), pEvent, LCARS.MAIN_MENU_TOP_BUTTON_WIDTH, LCARS.MAIN_MENU_TOP_BUTTON_HEIGHT
    )
...
def SaveConfig(pPane, pEvent):
    ...
    if (
        App.DamageableObject_IsDamageGeometryEnabled()
        and App.DamageableObject_IsVolumeDamageGeometryEnabled()
        and App.DamageableObject_IsBreakableComponentsEnabled()
    ):
        App.g_kConfigMapping.SetIntValue("Graphics Options", "Visible Damage", 3)
    elif (
        App.DamageableObject_IsDamageGeometryEnabled()
        and App.DamageableObject_IsVolumeDamageGeometryEnabled()
        and not App.DamageableObject_IsBreakableComponentsEnabled()
    ):
        App.g_kConfigMapping.SetIntValue("Graphics Options", "Visible Damage", 2)
    elif (
        App.DamageableObject_IsDamageGeometryEnabled()
        and not App.DamageableObject_IsVolumeDamageGeometryEnabled()
        and not App.DamageableObject_IsBreakableComponentsEnabled()
    ):
...
def BuildConfigureGeneralTab(pOptionsPane, pContentPane, bGameEnded=0):
    ...
    pOptionsPane.AddChild(
        CreateMenuYesNoButton(pDatabase.GetString("Character Tool Tips"), ET_TOOL_TIP_TOGGLE, App.CharacterClass_AreToolTipsEnabled(), fMenuWidth)
    )
    pOptionsPane.AddChild(
        CreateMenuYesNoButton(pDatabase.GetString("Collision Alert"), ET_COLLISION_ALERT_TOGGLE, App.CharacterClass_IsCollisionAlertEnabled(), fMenuWidth)
    )
...
def BuildConfigureMutatorsTab(pOptionsPane, pContentPane, bGameEnded=0):
    ...
    pContentWindow = App.STStylizedWindow_Create(
        "StylizedWindow", "NoMinimize", "", 0.0, 0.0, App.STSubPane_Create(LCARS.MAIN_MENU_MIDDLE_CONTENT_WIDTH, LCARS.MAIN_MENU_MIDDLE_CONTENT_HEIGHT)
    )
...
def GetGraphicMasterSetting():
    ...
    if (
        App.g_kLODModelManager.GetDropLODLevel() == 0
        ...
        and App.g_kImageManager.GetImageDetail() == 2
        and App.DamageableObject_IsDamageGeometryEnabled() == 1
        ...
        App.g_kLODModelManager.GetDropLODLevel() == 1
        and App.g_kImageManager.AreMipMapsEnabled() == 1
        and App.g_kLODModelManager.AreGlowMapsEnabled() == 1
        and App.SpaceCamera_IsSpaceDustEnabledInGame() == 1
        and App.g_kImageManager.GetImageDetail() == 1
        and App.DamageableObject_IsDamageGeometryEnabled() == 1
        ...
        App.g_kLODModelManager.GetDropLODLevel() == 1
        and App.g_kImageManager.AreMipMapsEnabled() == 1
        and App.g_kLODModelManager.AreGlowMapsEnabled() == 0
        and App.SpaceCamera_IsSpaceDustEnabledInGame() == 0
        and App.g_kLODModelManager.AreSpecularMapsEnabled() == 0
        and App.g_kLODModelManager.IsMotionBlurEnabled() == 0
        and App.g_kImageManager.GetImageDetail() == 0
        and App.DamageableObject_IsDamageGeometryEnabled() == 0
        and App.DamageableObject_IsVolumeDamageGeometryEnabled() == 0
        and App.DamageableObject_IsBreakableComponentsEnabled() == 0
    ):
...
def BuildConfigureGraphicsTab(pOptionsPane, pContentPane, bGameEnded=0):
    ...
    kMasterToggles = (
        (pDatabase.GetString("HIGH  MED [LOW] CUSTOM"), ET_GRAPHICS_OPTIONS_MASTER, 0),
    ...
    kLODSkipToggles = (
        (pDatabase.GetString("[HIGH] MED  LOW"), ET_GRAPHICS_OPTIONS_LOD, 0),
    ...
    kTextureDetailToggles = (
        (pDatabase.GetString("HIGH  MED [LOW]"), ET_GRAPHICS_OPTIONS_TEXTURE_DETAIL, 0),
    ...
    kVisibleDamageToggles = (
        (pDatabase.GetString("ON [OFF]"), ET_GRAPHICS_OPTIONS_VISIBLE_DAMAGE, 0),
    ...
    pContentWindow.AddChild(
        CreateMenuYesNoButton(pDatabase.GetString("Specular Highlights"), ET_GRAPHICS_OPTIONS_SPECULAR, iSpecularOption, fMenuWidth), 0, 0, 0
    )
...
def CreateTitleNameHorizCredit(sTitle, sName, pSequence, fPos=15, fDelay=0.0, iFontIndex=2):
    ...
    if sTitleString:
        pAction = App.TGCreditAction_Create(
            sTitleString,
            pCreditPane,
            g_fTitleLeft,
            fHeight,
            g_fCreditDelay - fDelay,
            0.25,
            0.5,
            iFontSize,
    ...
    if sName:
        pAction = App.TGCreditAction_CreateSTR(
            sName,
            pCreditPane,
            g_fNameLeft,
            fHeight,
...
def AddActivisionCredits(pSequence):
    ...
    pSubSeq = App.TGSequence_Create()
    pSubSeq.SetUseRealTime(1)
    CreateTitleNameHorizCredit("QA PL", "Si�n Rodriguez Y Gibson", pSubSeq, 8, 0.00)
...
def HandleSwitchIconGroups(pObject, pEvent):
    ...
    if pEvent:
        App.InterfaceModule_SwitchFonts(
            App.g_kFontManager.GetFontGroup(g_pcSmallFont, g_kFlightSmallFontSize[pEvent.GetInt()]),
...
def QuickBattleHandler(pOptionsWindow, pEvent):
    ...
    if pGame and not App.g_kUtopiaModule.IsMultiplayer():
        ...
        pModalDialogWindow.Run(
            None, pDatabase.GetString("Are you sure you want to quit"), pDatabase.GetString("OK"), pOkEvent, pDatabase.GetString("Cancel"), None
        )
