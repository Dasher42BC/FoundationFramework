...
NonSerializedObjects = ("dFirePrim",)
...
##########################################################
# An Propertied object is a type of object that carries a minimum of local
# variables, deferring to its prototype calls for most data members.  It is
# also meant to use a copy-on-write scheme for updating member objects.
##########################################################
class Propertied:
...
##########################################################
# Akin to a Mutator, this holds list of Technology Elements that have
# different effects.  It inherits OverrideDef's ability to participate in
# the organized creation and destruction of game modes associated with
# mutators.  As a subclass of Registry, it is a master broker of active
...
class ShipInstance(Propertied, FTBEventUser):
        ...
        if proto and proto.__dict__.has_key("dTechs"):
            ...
            for i in proto.dTechs.keys():
                if oTechs._keyList.has_key(i):
                    ...
                    debug("Tech: %s isn't installed::%s" % (i, sName))
    ...
    def Disable(self, pShip, pSubSys, time, bForce=0):
        ...
        if self.dDisabled.has_key(sSubSys):
            ...
            if pSubSys == pShields:
                tShieldDirs = (
                    pShields.GetCurShields(pShields.FRONT_SHIELDS),
                    pShields.GetCurShields(pShields.REAR_SHIELDS),
                    pShields.GetCurShields(pShields.TOP_SHIELDS),
                    pShields.GetCurShields(pShields.BOTTOM_SHIELDS),
                    pShields.GetCurShields(pShields.LEFT_SHIELDS),
                    pShields.GetCurShields(pShields.RIGHT_SHIELDS),
                )
...
class InacAITrigger(TriggerDef):
    ...
    def __call__(self, pObject, pEvent):
        ...
        if not pTarget or not pShip or not pWeapon:
            return
...
class PlayerPrimaryWeaponTriggerDef(Foundation.TriggerDef):
    def __call__(self, pObject, pEvent):
        ...
        bPlayerPrimaryWeapon = pEvent.GetBool()
        pObject.CallNextHandler(pEvent)


# PlayerPrimaryWeaponTriggerDef('PlayerPrimaryWeapon', App.ET_INPUT_FIRE_PRIMARY)
