...
dTORP_X_BIAS = {640: 1, 800: 1, 1024: 2, 1280: 3, 1366: 3, 1600: 3, 1920: 3, 2048: 3, 2560: 3}
dTORP_Y_BIAS = {640: -7, 800: -7, 1024: -7, 1280: -7, 1366: -7, 1600: -7, 1920: -7, 2048: -7, 2560: -7}
dPHASER_X_BIAS = {640: 1, 800: 1, 1024: 2, 1280: 3, 1366: 3, 1600: 3, 1920: 3, 2048: 3, 2560: 3}
dPHASER_Y_BIAS = {640: -8, 800: -8, 1024: -8, 1280: -8, 1366: -8, 1600: -8, 1920: -8, 2048: -8, 2560: -8}


def _FixWeaponsIconBias():
    pGraphicsMode = App.GraphicsModeInfo_GetCurrentMode()
    w = pGraphicsMode.GetWidth()

    if not dTORP_X_BIAS.has_key(w):
        dTORP_X_BIAS[w] = int(w / 1600 * dTORP_X_BIAS[1600])
    if not dTORP_Y_BIAS.has_key(w):
        dTORP_Y_BIAS[w] = dTORP_Y_BIAS[1600]
    if not dPHASER_X_BIAS.has_key(w):
        dPHASER_X_BIAS[w] = int(w / 1600 * dPHASER_X_BIAS[1600])
    if not dPHASER_Y_BIAS.has_key(w):
        dPHASER_Y_BIAS[w] = dPHASER_Y_BIAS[1600]
...
def Create(pDisplay, fWidth, fHeight):
    ...
    pDisplayPane.AddChild(
        pShipIcon, (pDisplay.GetWidth() / 2.0) - (pShipIcon.GetWidth() / 2.0), (pDisplay.GetHeight() / 2.0) - (pShipIcon.GetHeight() / 2.0), 0
    )
    ...
    pDisplay.SetFixedSize(LCARS.WEAPONS_PANE_WIDTH + pDisplay.GetBorderWidth(), LCARS.WEAPONS_PANE_HEIGHT + pDisplay.GetBorderHeight(), 0)
...
def CreateTorpedoPane(fWidth, fHeight, idShip):
    ...
    for i in range(pTorpedoSystem.GetNumChildSubsystems()):
        ...
        fPosY = (pProp.GetIconPositionY() + dTORP_Y_BIAS[pGraphicsMode.GetWidth()]) / pGraphicsMode.GetHeight()
...
# 	Return:	None
###############################################################################
def RepositionEnergyWeaponPanes(pSystem, pUpperPane, pUpperIndPane, pLowerPane, pLowerIndPane):
    # Get graphics mode.
    pGraphicsMode = App.GraphicsModeInfo_GetCurrentMode()
    ...
    if pShip and pSystem:
        ...
        for i in range(pSystem.GetNumChildSubsystems()):
            ...
            fPosX = (pProp.GetIconPositionX() + dPHASER_X_BIAS[pGraphicsMode.GetWidth()]) / pGraphicsMode.GetWidth()
            fPosY = (pProp.GetIconPositionY() + dPHASER_Y_BIAS[pGraphicsMode.GetWidth()]) / pGraphicsMode.GetHeight()

            # Reposition the icon for this weapon.
            pIcon.SetPosition(fPosX, fPosY, 0)

            fPosX = (pProp.GetIndicatorIconPositionX() + dPHASER_X_BIAS[pGraphicsMode.GetWidth()]) / pGraphicsMode.GetWidth()
            fPosY = (pProp.GetIndicatorIconPositionY() + dPHASER_Y_BIAS[pGraphicsMode.GetWidth()]) / pGraphicsMode.GetHeight()
...
def SetShipIcon(pDisplay):
    ...
    if pShip == None:
        ...
        pDisplayPane.InsertChild(
            App.WeaponsDisplay.SHIP_ICON,
            pNewShipIcon,
            (pDisplay.GetWidth() / 2.0) - (pNewShipIcon.GetWidth() / 2.0),
            (pDisplay.GetHeight() / 2.0) - (pNewShipIcon.GetHeight() / 2.0),
            0,
        )
    else:
