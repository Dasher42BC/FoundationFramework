...
def RepositionUI(pDisplay):
    ...
    pShipIcon.SetPosition((fWidth - pShipIcon.GetWidth()) / 2.0, (fHeight - pShipIcon.GetHeight()) / 2.0, 0)
    pNoTarget.SetPosition((fWidth - pNoTarget.GetWidth()) / 2.0, (fHeight - pNoTarget.GetHeight()) / 2.0, 0)
    pUnknownObj.SetPosition((fWidth - pUnknownObj.GetWidth()) / 2.0, (fHeight - pUnknownObj.GetHeight()) / 2.0, 0)
...
def SetShipIcon(pDisplay):
    ...
    if pPlayer:
        ...
        if idShip == App.NULL_ID:
            ...
            pNoTarget.SetPosition((fWidth - pNoTarget.GetWidth()) / 2.0, (fHeight - pNoTarget.GetHeight()) / 2.0)
            ...
            if pPlayer.GetSensorSubsystem().IsObjectKnown(pPlayer.GetTarget()) == 0:
                HideShipIcons(pDisplay)
                DamageDisplay.HideIcons(pParent.GetDamageDisplay())
                pUnknownObj.SetPosition((fWidth - pUnknownObj.GetWidth()) / 2.0, (fHeight - pUnknownObj.GetHeight()) / 2.0)
    ...
    pDisplayPane.InsertChild(App.ShieldsDisplay.SHIP_ICON, pNewShipIcon, (fWidth - pNewShipIcon.GetWidth()) / 2.0, (fHeight - pNewShipIcon.GetHeight()) / 2.0)
